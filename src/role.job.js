...
const roleJob = {
    ...
    */
    ...
    ScontainerHarvestBehavior:function(creep,absolute = false){
        ...
        const roomName = creep.room.name
        let feedBack = JobERR
        if (!helpFunc.inArr(roomName,Game.spawns['Origin'].memory.infoRooms.controlled)) return feedBack
    ...
    SmineralHarvestBehavior:function(creep,absolute = false){
        ...
        if ((!creep.memory.signals.mineralTransfer && Game.getObjectById(creep.memory.mineralId).mineralAmount !== 0 &&
            creep.memory.getFreeCapacity() > 0) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    SlinkUpdateHarvestBehavior:function(creep,absolute = false){
        ...
        if ((LinksUpdate.length > 0 && !creep.memory.signals.building && creep.memory.getFreeCapacity() > 0) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    ScompoundMarketRetrieveBehavior:function(creep,absolute = false){
        ...
        if (_terminal.getFreeCapacity() === 0) return feedBack
    ...
    compoundMarketTransferBehavior:function(creep,absolute = false){
        ...
        if ((creep.memory.hasGoods && creep.memory.signals.marketTarget) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
            const _feedBack = creep.transfer(_terminal,creep.memory.marketTarget[1])
            if (_feedBack === ERR_NOT_ENOUGH_RESOURCES){
                creep.moveTo(_terminal)
            }else if (_feedBack === OK){
                creep.hasGoods = false
