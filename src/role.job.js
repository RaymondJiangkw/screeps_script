...
const roleJob = {
    ...
    */
    ...
    SpickUpBehavior:function(creep, absolute = false){
        ...
        if (creep.memory.signals.hasPickUp && 
            (creep.store.getFreeCapacity() === 0 || 
            targetDroppedResources.length + targetTombStones.length == 0)){
            creep.memory.signals.storing = true
            creep.memory.signals.hasPickUp = false
        }
        if ((!creep.memory.signals.storing && targetDroppedResources.length + targetTombStones.length > 0) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
            ...
            if (targetTombStones.length > 0){
                ...
                if (helpFunc.creepWithdrawAll(creep.id,targetPickUpObject.id) === ERR_NOT_IN_RANGE){
                    ...
                    creep.memory.signals.hasPickUp = true
                }
    ...
    chargeLinkBehavior:function(creep, absolute = false){
        ...
        if (feedBack === JobOK){
            ...
            for (let i = 0; i < Game.spawns['Origin'].memory.init.groupedLinks.emitFrom[roomName].resources.length;i++){
                ...
                if (targetLink.store.getFreeCapacity() > 0){
    ...
    SmineralHarvestBehavior:function(creep,absolute = false){
        ...
        if ((!creep.memory.signals.mineralTransfer && Game.getObjectById(creep.memory.mineralId).mineralAmount !== 0) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    SlinkUpdateHarvestBehavior:function(creep,absolute = false){
        ...
        if ((LinksUpdate.length > 0 && !creep.memory.signals.building) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    SlinkStorageHarvestBehavior:function(creep,absolute = false){
        ...
        if ((LinksBackUp.length > 0 && !creep.memory.signals.building) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    SmineralContainerHarvestBehavior:function(creep, absolute = false){
        ...
        creep.memory.iftransfering && !creep.memory.signals.mineralTransfer) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    ScompoundLabRetrieveBehavior:function(creep,absolute = false){
        ...
        if ((creep.memory.targetResource !== undefined &&
             !creep.memory.signals.labTransfer) || absolute === true){
            feedBack = JobOK
        }
        if (feedBack === JobOK){
    ...
    run:function(object, type = 'creep'){
        ...
        if (type === 'creep'){ // Case 1: creep
            ...
            if (!creep.memory.bodyParts){
                helpFunc.getCreepBody(creep)
            }
            if (creep.memory.signals === undefined){
                creep.memory.signals = {}
            }
            // Prepare the boosts
        ...
        for (let i = 0; i < taskList.length; ++i){
            if (taskList[i][0] === 'S' && type === 'creep' && signalResponded === false){
                continue
            }
