...
module.exports = function(){
    for (var roomName of claimedRooms){
        ...
        if (Memory.info.roomState[roomName] === "neutral") {
            ...
            if (home) Game.rooms[home].AddAttackTask("claim",undefined,roomName,utils.getSuitableRoute(home,roomName),1);
        }else if (Memory.info.roomState[roomName] === false){
            var home = utils.getClosetSuitableRoom(roomName,7,true);
            if (home) Game.rooms[home].AddAttackTask("attack",undefined,roomName,utils.getSuitableRoute(home,roomName));
        }
    }
