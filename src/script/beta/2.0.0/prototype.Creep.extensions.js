...
const creepsExtensions = {
    ...
    _getEnergy(identity,subIdentity,roomName,amount) {
        ...
        const hasEnergyContainers         = _.filter(room.containers, c => c.store[RESOURCE_ENERGY] > 10).sort(posCmp);
        ...
        if (!chosenObject) chosenObject = hasEnoughEnergyContainers[0] || droppedEnergys[0] || hasEnergyContainers[0] || hasEnergy(room.storage) || hasEnergy(room.terminal);
